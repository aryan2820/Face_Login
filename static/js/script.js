...
function init() {
    ...
    navigator.mediaDevices.getUserMedia({ video: true })
        ...
        .catch(error => {
            ...
            alert('Unable to access webcam. Please make sure you grant permission to access the camera.');
        });
}
...
function register() {
    ...
    if (!name || !photo) {
        alert('Name and face photo must be filled out!');
        return;
    }
...
function login() {
    ...
    if (!photo) {
        alert('Capture a photo first before logging in.');
        return;
    }
